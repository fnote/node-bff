    ...
    getAuthorizedBusinessUnits = (opcoAttributeBunit, userRole) => {
        ...
        if (!opcoAttributeBunit || !userRole) {
            ...
            if (matchedValidBusinessUnitList.length > 0) {
                const authorizedPricingTransformationEnabledBunitList = this.matchedPricingTransformationEnabledBusinessUnit(opcoAttributeBunit);

                if (authorizedPricingTransformationEnabledBunitList.length > 0) {
